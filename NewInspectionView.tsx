    ...
    try {
      ...
      if (apiSuccess && analysisResult?.data) {
        ...
        const detectedCommodity =
          resData.commodityName && resData.commodityName !== 'Not detected'
            ? resData.commodityName
            : commodityName || 'Not detected';
        const detectedBrand = resData.brand || 'Not detected';
        const detectedMfg = resData.manufacturer || 'Not detected';
        const detectedMfgAddr = resData.mfgAddress || 'Not detected';
        const detectedOrigin = resData.countryOfOrigin || 'Not detected';
        const detectedNetQty = resData.netQuantity || 'Not detected';
        const detectedMrp = resData.mrp || 'Not detected';
        const detectedUsp = resData.unitSalePrice || 'Not detected';
        const detectedMfgDate = resData.mfgDate || 'Not detected';
        const detectedExpDate = resData.expiryDate || 'Not detected';
        const detectedCare = resData.consumerCare || 'Not detected';

        setCommodityName(detectedCommodity !== 'Not detected' ? detectedCommodity : '');
        setBrand(detectedBrand !== 'Not detected' ? detectedBrand : '');
        setManufacturer(detectedMfg !== 'Not detected' ? detectedMfg : '');
        setMfgAddress(detectedMfgAddr !== 'Not detected' ? detectedMfgAddr : '');
        setCountryOfOrigin(detectedOrigin !== 'Not detected' ? detectedOrigin : '');
        setNetQuantity(detectedNetQty !== 'Not detected' ? detectedNetQty : '');
        setMrp(detectedMrp !== 'Not detected' ? detectedMrp : '');
        setUnitSalePrice(detectedUsp !== 'Not detected' ? detectedUsp : '');
        setMfgDate(detectedMfgDate !== 'Not detected' ? detectedMfgDate : '');
        setExpiryDate(detectedExpDate !== 'Not detected' ? detectedExpDate : '');
        setConsumerCare(detectedCare !== 'Not detected' ? detectedCare : '');

        // Run real Legal Metrology compliance evaluation
        ...
        // In case of extraction failure or missing API key, set current/empty state and evaluate
        const evaluation = evaluateMetrologyCompliance({
